...
public class FogOfWarManager : MonoBehaviour
{
    ...
    public void ShowFaction(FogOfWar.Players _Faction)
    {
        int factionInt = Mathf.RoundToInt(Mathf.Log((float)(int)_Faction, 2f));

        if (factionInt >= Factions.Count)
        {
            Debug.LogWarning("[UFoW] Cant reveal Faction, because it does not exist. Create more Factions.");
            return;
        }

        RevealFaction = _Faction;


        if (BlurFog == FogOfWar.FogQuality.Off)
        {
            Shader.SetGlobalTexture("FogOfWar", FogOfWar.Factions[factionInt].FogTexture);
        }
        else
        {
            if (fogOfWarBlur == null)
            ...
            Shader.SetGlobalTexture("FogOfWar", FogOfWar.Factions[factionInt].BluredRenderTexture);
        }

        FogOfWar.RevealFactionInt = factionInt;
        FogOfWar.RevealFaction = RevealFaction;
    ...
    public void SetResistanceMap(string _ResistanceMapPath)
    {
#if UNITY_EDITOR
        if (Resistance == null && _ResistanceMapPath != "")
        {
            Resistance = UnityEditor.AssetDatabase.LoadAssetAtPath<Texture2D>(_ResistanceMapPath);
        }
#endif
        if (Resistance != null)
        {
            FogOfWar.ResistanceMapData = Resistance.GetPixels();
        }
        else
        {
            Debug.LogWarning("Resistance map not assigned! Use Fog manually.");
        }
    }

    public void SetResistanceMapData(Color[] _ResistanceMapData)
    {
        FogOfWar.ResistanceMapData = _ResistanceMapData;
#if UNITY_EDITOR
        Texture2D tex = new Texture2D(LevelWidth, LevelHeight);
        tex.SetPixels(_ResistanceMapData);
        tex.Apply(false);
        Resistance = tex;
#endif
    }
    ...
    public Texture2D GetResistanceMap()
    {
        if (FogOfWar.ResistanceMapData == null || Resistance == null)
        {
            return null;
        }
        Resistance.SetPixels(FogOfWar.ResistanceMapData);
        Resistance.Apply(false);
        return Resistance;
    }
    ...
    public void PreparePlacement()
    {
        if (PlacementID == null || PlacementID.Length != LevelWidth * LevelHeight)
        {
            ...
            {
                PlacementID[i] = Mathf.FloorToInt(FogOfWar.ResistanceMapData[i].a * 255);
            }
        }
    ...
    public void FinalizePlacement()
    {
        for (int i = 0; i < PlacementID.Length; i++)
        {
            FogOfWar.ResistanceMapData[i].a = (float)PlacementID[i] / 255f;
        }
    }
        ...
        if (fogAlignment == FogOfWar.FogAlignment.Horizontal)
        {
            ...
            for (float x = StartX; x <= EndX; x++)
            {
                for (float z = StartZ; z <= EndZ; z++)
                {
                    ...
                    FogOfWar.ResistanceMapData[coord].r = (float)ID / 255f;
                }
            }
        }
        else
    ...
    {//클릭해서 heightMap 바꿀 때, 직접적으로 값을 바꾸는 코드
        if (fogAlignment == FogOfWar.FogAlignment.Horizontal)
        {
            int coord = Mathf.FloorToInt(Mathf.Floor((_Position.z - Origin.z) / PatchScale) * LevelWidth + Mathf.Floor((_Position.x - Origin.x) / PatchScale));
            ...
            FogOfWar.ResistanceMapData[coord].g = (1f / 255f) * _Height;
        }
        else
        {
            int coord = Mathf.FloorToInt(Mathf.Floor((_Position.y - Origin.y) / PatchScale) * LevelWidth + Mathf.Floor((_Position.x - Origin.x) / PatchScale));
