...
public static class FogOfWar
{
    ...
    public static Color[] ResistanceMapData;
    ...
    {
        Factions.Clear();
        FogOfWarRevealer.Clear();
        FogOfWarRevealerPositions.Clear();
        FogOfWarVisionBlocker.Clear();
        FogOfWarVisionBlockerPositions.Clear();
    }
    ...
    public static void PrepareThread()
    {
        if (ScalePatchModifier)
        {
            ...
            {
                FogOfWarRevealerPositions[i] = (FogOfWarRevealer[i].sceneReference.transform.position - Origin) / PatchScale;
            }
        ...
        else
        {
            ...
            for (int i = 0; i < RevealerCount; i++)
            {
                FogOfWarRevealerPositions[i] = FogOfWarRevealer[i].sceneReference.transform.position;
            }
    ...
    public static void UnRegisterRevealer(int ID)
    {
        ...
        {
            if (ID == FogOfWarRevealer[i].sceneReference.GetInstanceID())
            {
    ...
    public static void SetResistanceMapData(Color[] _ResistanceMapData)
    {
        ResistanceMapData = _ResistanceMapData;
    }

    public static void PrepareDynamicBlockers()
    {
        int RMDLength = ResistanceMapData.Length;

        for (int i = 0; i < RMDLength; i++)
        {
            ResistanceMapData[i].b = 0.0f;
        }
        ...
        foreach (Vector3 v in FogOfWarVisionBlockerPositions)
        {
            if (fogAlignment == FogOfWar.FogAlignment.Horizontal)
            {
                ...
                if (NewCoordinate < RMDLength && NewCoordinate >= 0)
                {
                    ResistanceMapData[NewCoordinate].b = 1.0f;
                }
            }
    ...
    {
        if (ResistanceMapData[_Coordinate].r != 0.0f)
            return true;

        if (ResistanceMapData[_Coordinate].g - ResistanceMapData[_StartCoordinate].g > _UpVision)
            return true;

        if (ResistanceMapData[_Coordinate].b != 0.0f)
            return true;

        if (ResistanceMapData[_Coordinate].a != ResistanceMapData[_StartCoordinate].a && ResistanceMapData[_Coordinate].a != 1f)
            return true;
    ...
    public static bool IsPositionRevealedByFaction(Vector3 _Position, int _RevealingFaction)
    {
        if (ScalePatchModifier)
        {
        ...
        if (Factions[_RevealingFaction].FogOfWarMapData[Coordinate].r >= 1f)
        {
            return true;
        }
        else
