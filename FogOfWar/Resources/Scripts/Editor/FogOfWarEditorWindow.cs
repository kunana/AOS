...
public class FogOfWarEditorWindow : EditorWindow
{
    ...
    private bool BorderResistanceMap = true;
    ...
    void OnEnable()
    {
		...
		if (TerrainMatPreview == null)
			TerrainMatPreview = new Material(Shader.Find("Hidden/UFoW/Debug/GUITexture"));

		if (DDDTexture == null)
			DDDTexture = Resources.Load("Textures/Editor/DDD") as Texture2D;
	}
            ...
            EditorGUILayout.BeginFadeGroup(Group0.faded);
            {
                EditorGUILayout.BeginVertical("Button");
                {
                    ...
                    EditorGUILayout.BeginHorizontal();
                    {
                        ...
                        {
                            Rect rect = GUILayoutUtility.GetRect(Screen.width / 3f - 25f, 150f);

                            if (GUILayout.Button("Horizontal (Terrain)", GUILayout.MinWidth(150f), GUILayout.MinHeight(50f)))
                            {
                                MakeTerrain = true;
                                if (CurrentStage == 0)
                                    CurrentStage++;
                        ...
                        {
                            Rect rect = GUILayoutUtility.GetRect(Screen.width / 3f - 25f, 150f);
                            if (GUILayout.Button("Vertical (Canvas)", GUILayout.MinWidth(150f), GUILayout.MinHeight(50f)))
                            {
                                MakeTerrain = false;
                                if (CurrentStage == 0)
                                    CurrentStage++;
						...
						{
							Rect rect = GUILayoutUtility.GetRect(Screen.width / 3f - 25f, 150f);
							if (GUILayout.Button("3D (GPU Fog of War)", GUILayout.MinWidth(150f), GUILayout.MinHeight(50f)))
							{
								GameObject g = new GameObject("3D Fog of War Manager");
								g.AddComponent<FogOfWar3DManager>();
								this.Close();
							}
							EditorGUI.DrawPreviewTexture(rect, DDDTexture, TerrainMatPreview, ScaleMode.ScaleAndCrop);

							EditorGUILayout.LabelField("In this mode, the Fog of War is simulated in all 3 dimensions and calculated on the GPU.", EditorStyles.helpBox);
						}
						EditorGUILayout.EndVertical();
					}
                    EditorGUILayout.EndHorizontal();
                ...
                EditorGUILayout.BeginFadeGroup(Group1.faded);
                {
                        ...
                        EditorGUILayout.BeginVertical("Box");
                    {
                        ...
                        EditorGUILayout.BeginHorizontal();
                        {
                            ...
                            BorderResistanceMap = EditorGUILayout.Toggle(BorderResistanceMap);
                        }
                        EditorGUILayout.EndHorizontal();
                    }
                    EditorGUILayout.EndVertical();
                ...
                EditorGUILayout.BeginVertical("Button");
                {
                    ...
                    EditorGUILayout.BeginHorizontal();
                    {
                        if (GUILayout.Button(">Pick<", EditorStyles.toolbarButton, GUILayout.MaxWidth(Screen.width * 0.3f)))
                        {
                            ...
                            if (FileLocation != "")
                            {
                                if (!FileLocation.Contains("_ResistanceMap"))
                                    FileLocation = FileLocation.Insert(FileLocation.Length - 4, "_ResistanceMap");
                            }
                        }
            ...
            if (CurrentStage == 2)
            {
                EditorGUILayout.BeginHorizontal();
                {
                    ...
                    if (GUILayout.Button("Create |>"))
                    {
                        ...
                        {
                            Debug.LogWarning("[UFoW] You must select a file path first!");
                        }
                    }
                    GUI.color = Color.white;
                }
