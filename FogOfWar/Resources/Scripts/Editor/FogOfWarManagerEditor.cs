...
public class FogOfWarManagerEditor : Editor
{
    ...
    SerializedProperty ResistanceMapPrefab;
#if !UNITY_WEBGL
...
#pragma warning restore 414
    ...
    void OnEnable()
    {
        ...
        ResistanceMapPrefab = serializedObject.FindProperty("Resistance");
#if !UNITY_WEBGL
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (DebugMatR == null)
        {
            DebugMatR = new Material(Shader.Find("Hidden/UFoW/Debug/GUIDebug_R"));
        }

        if (DebugMatG == null)
        {
            DebugMatG = new Material(Shader.Find("Hidden/UFoW/Debug/GUIDebug_G"));
        }
        /*
        if (DebugMatB == null)
        {
            DebugMatB = new Material(Shader.Find("Hidden/UFoW/Debug/GUIDebug_B"));
        }
        */
        if (DebugMatA == null)
        {
            DebugMatA = new Material(Shader.Find("Hidden/UFoW/Debug/GUIDebug_A"));
        }
            ...
            if (fogOfWarManager.ShowPreview.target)
            {
                EditorGUILayout.BeginFadeGroup(fogOfWarManager.ShowPreview.faded);
                {
                    ...
                    EditorGUILayout.BeginHorizontal();
                    {
                        ...
                        if (ResistanceMap != null && fogOfWarManager.ShowPreview.faded == 1f)
                        {
                            if (DisplayTexA)
                            {
                                EditorGUI.DrawPreviewTexture(rect, ResistanceMap, DebugMatR, ScaleMode.ScaleToFit);
                            }

                            if (DisplayTexB)
                            {
                                EditorGUI.DrawPreviewTexture(rect, ResistanceMap, DebugMatG, ScaleMode.ScaleToFit);
                            }

                            if (DisplayTexC)
                            {
                                EditorGUI.DrawPreviewTexture(rect, ResistanceMap, DebugMatA, ScaleMode.ScaleToFit);
                            }

                            if (DisplayTexD)
                            {
                                if (FogOfWar.Factions != null && FogOfWar.Factions.Count > 0)
                                {
            ...
            EditorGUILayout.BeginVertical("Box");
            {
                ...
                if (fogOfWarManager.ShowLevelOption.target)
                {
                    EditorGUILayout.BeginFadeGroup(fogOfWarManager.ShowLevelOption.faded);
                    {
                        ...
                        ResistanceMapPrefab.objectReferenceValue = EditorGUILayout.ObjectField(ResistanceMapPrefab.objectReferenceValue, typeof(Texture2D), false) as Texture2D;
        ...
        {
            fogOfWarManager.SetUpShaderKeywords();
    ...
    void Update()
    {
        ...
        ResistanceMap = fogOfWarManager.GetResistanceMap() as Texture;
        ...
        if (fogOfWarManager != null && fogOfWarManager.DebugMode)
        {
            if (fogOfWarManager.fogAlignment == FogOfWar.FogAlignment.Horizontal)
            {
                ...
                if (Physics.Raycast(ray, out rayHit))
                {
                    if (rayHit.point.x >= Origin.vector3Value.x && rayHit.point.x <= fogOfWarManager.LevelWidth * PatchScale.floatValue + Origin.vector3Value.x && rayHit.point.z >= Origin.vector3Value.z && rayHit.point.z <= fogOfWarManager.LevelHeight * PatchScale.floatValue + Origin.vector3Value.z)
                    {
                        ...
                        if (placeMode.enumValueIndex == 0 && fogOfWarManager.ShowGrassOptions.target)
                        {
                            ...
                            for (float x = StartX; x <= EndX; x++)
                            {
                                ...
                                {
                                    Handles.RectangleHandleCap(0, new Vector3(x, rayHit.point.y, z), Quaternion.Euler(90f, 0f, 0f), .5f * PatchScale.floatValue, EventType.MouseMove);
                                    Handles.RectangleHandleCap(0, new Vector3(x, rayHit.point.y, z), Quaternion.Euler(90f, 0f, 0f), .49f * PatchScale.floatValue, EventType.MouseMove);
                                    Handles.RectangleHandleCap(0, new Vector3(x, rayHit.point.y, z), Quaternion.Euler(90f, 0f, 0f), .48f * PatchScale.floatValue, EventType.MouseMove);

                                    Handles.color = new Color(.5f, .2f, .0f, 1f);
                                    Handles.RectangleHandleCap(0, new Vector3(x, rayHit.point.y, z), Quaternion.Euler(90f, 0f, 0f), .47f * PatchScale.floatValue, EventType.MouseMove);
                                    Handles.RectangleHandleCap(0, new Vector3(x, rayHit.point.y, z), Quaternion.Euler(90f, 0f, 0f), .46f * PatchScale.floatValue, EventType.MouseMove);
                                    Handles.RectangleHandleCap(0, new Vector3(x, rayHit.point.y, z), Quaternion.Euler(90f, 0f, 0f), .45f * PatchScale.floatValue, EventType.MouseMove);
                                }
                            }
                        ...
                        {
                            Handles.RectangleHandleCap(0, new Vector3(f1, rayHit.point.y, f2), Quaternion.Euler(90f, 0f, 0f), .5f * PatchScale.floatValue, EventType.MouseMove);
                            Handles.RectangleHandleCap(0, new Vector3(f1, rayHit.point.y, f2), Quaternion.Euler(90f, 0f, 0f), .49f * PatchScale.floatValue, EventType.MouseMove);
                            Handles.RectangleHandleCap(0, new Vector3(f1, rayHit.point.y, f2), Quaternion.Euler(90f, 0f, 0f), .48f * PatchScale.floatValue, EventType.MouseMove);

                            Handles.color = new Color(.953f, .659f, .157f, 1f);
                            Handles.RectangleHandleCap(0, new Vector3(f1, rayHit.point.y, f2), Quaternion.Euler(90f, 0f, 0f), .47f * PatchScale.floatValue, EventType.MouseMove);
                            Handles.RectangleHandleCap(0, new Vector3(f1, rayHit.point.y, f2), Quaternion.Euler(90f, 0f, 0f), .46f * PatchScale.floatValue, EventType.MouseMove);
                            Handles.RectangleHandleCap(0, new Vector3(f1, rayHit.point.y, f2), Quaternion.Euler(90f, 0f, 0f), .45f * PatchScale.floatValue, EventType.MouseMove);
                        }
                        ...
                        Handles.ConeHandleCap(0, new Vector3(-1f + Origin.vector3Value.x, rayHit.point.y, rayHit.point.z), Quaternion.Euler(new Vector3(0f, 90f, 0f)), 1f, EventType.MouseMove);
                        Handles.ConeHandleCap(0, new Vector3(fogOfWarManager.LevelWidth * PatchScale.floatValue + Origin.vector3Value.x + 1f, rayHit.point.y, rayHit.point.z), Quaternion.Euler(new Vector3(0f, -90f, 0f)), 1f, EventType.MouseMove);

                        Handles.color = Color.blue;

                        Handles.ConeHandleCap(0, new Vector3(rayHit.point.x, rayHit.point.y, -1f + Origin.vector3Value.z), Quaternion.Euler(new Vector3(0f, 0f, 0f)), 1f, EventType.MouseMove);
                        Handles.ConeHandleCap(0, new Vector3(rayHit.point.x, rayHit.point.y, fogOfWarManager.LevelWidth * PatchScale.floatValue + Origin.vector3Value.z + 1f), Quaternion.Euler(new Vector3(0f, 180f, 0f)), 1f, EventType.MouseMove);
                        Handles.color = Color.white;
                    }
                    ...
                    if (NewCoordinate < FogOfWar.ResistanceMapData.Length && NewCoordinate >= 0)
                    {
                        ...
                        if (FogOfWar.ResistanceMapData[NewCoordinate].r == 0f)
                        {
                        ...
                        {
                            int ID = Mathf.RoundToInt(FogOfWar.ResistanceMapData[NewCoordinate].r * 255f);
                        ...
                        Handles.Label(rayHit.point + Camera.current.transform.rotation * new Vector3(0.025f, 0.061f, 0f) * CamDist, "Height: " + Mathf.RoundToInt(FogOfWar.ResistanceMapData[NewCoordinate].g * 255f).ToString(), EditorStyles.whiteMiniLabel);
                        Handles.Label(rayHit.point + Camera.current.transform.rotation * new Vector3(0.025f, 0.044f, 0f) * CamDist, "Vision modifier: " + Mathf.FloorToInt(FogOfWar.ResistanceMapData[NewCoordinate].a * 255), EditorStyles.whiteMiniLabel);
                    }
                }
            ...
            if (fogOfWarManager.fogAlignment == FogOfWar.FogAlignment.Vertical)
            {
                ...
                if (Physics.Raycast(ray, out rayHit))
                {
                    ...
                    if (rayHit.point.x >= Origin.vector3Value.x && rayHit.point.x <= fogOfWarManager.LevelWidth * PatchScale.floatValue + Origin.vector3Value.x && rayHit.point.y >= Origin.vector3Value.y && rayHit.point.y <= fogOfWarManager.LevelHeight * PatchScale.floatValue + Origin.vector3Value.y)
                    {
                        ...
                        if (placeMode.enumValueIndex == 0 && fogOfWarManager.ShowGrassOptions.target)
                        {
                            ...
                            for (float x = StartX; x <= EndX; x++)
                            {
                                ...
                                {
                                    Handles.RectangleHandleCap(0, new Vector3(x, y, rayHit.point.z), Quaternion.identity, .5f * PatchScale.floatValue, EventType.MouseMove);
                                    Handles.RectangleHandleCap(0, new Vector3(x, y, rayHit.point.z), Quaternion.identity, .49f * PatchScale.floatValue, EventType.MouseMove);
                                    Handles.RectangleHandleCap(0, new Vector3(x, y, rayHit.point.z), Quaternion.identity, .48f * PatchScale.floatValue, EventType.MouseMove);

                                    Handles.color = new Color(.5f, .2f, .0f, 1f);
                                    Handles.RectangleHandleCap(0, new Vector3(x, y, rayHit.point.z), Quaternion.identity, .47f * PatchScale.floatValue, EventType.MouseMove);
                                    Handles.RectangleHandleCap(0, new Vector3(x, y, rayHit.point.z), Quaternion.identity, .46f * PatchScale.floatValue, EventType.MouseMove);
                                    Handles.RectangleHandleCap(0, new Vector3(x, y, rayHit.point.z), Quaternion.identity, .45f * PatchScale.floatValue, EventType.MouseMove);
                                }
                            }
                        }
                        else
                        {
                            Handles.RectangleHandleCap(0, new Vector3(f1, f2, rayHit.point.z), Quaternion.identity, .5f * PatchScale.floatValue, EventType.MouseMove);
                            Handles.RectangleHandleCap(0, new Vector3(f1, f2, rayHit.point.z), Quaternion.identity, .49f * PatchScale.floatValue, EventType.MouseMove);
                            Handles.RectangleHandleCap(0, new Vector3(f1, f2, rayHit.point.z), Quaternion.identity, .48f * PatchScale.floatValue, EventType.MouseMove);

                            Handles.color = new Color(.953f, .659f, .157f, 1f);
                            Handles.RectangleHandleCap(0, new Vector3(f1, f2, rayHit.point.z), Quaternion.identity, .47f * PatchScale.floatValue, EventType.MouseMove);
                            Handles.RectangleHandleCap(0, new Vector3(f1, f2, rayHit.point.z), Quaternion.identity, .46f * PatchScale.floatValue, EventType.MouseMove);
                            Handles.RectangleHandleCap(0, new Vector3(f1, f2, rayHit.point.z), Quaternion.identity, .45f * PatchScale.floatValue, EventType.MouseMove);
                        }

                        DrawPreviewFieldVertical(rayHit.point, placeMode.enumValueIndex);
                        Handles.color = Color.red;
                        Handles.ConeHandleCap(0, new Vector3(-1f + Origin.vector3Value.x, rayHit.point.y, rayHit.point.z), Quaternion.Euler(new Vector3(0f, 90f, 0f)), 1f, EventType.MouseMove);
                        Handles.ConeHandleCap(0, new Vector3(fogOfWarManager.LevelWidth * PatchScale.floatValue + Origin.vector3Value.y + 1f, rayHit.point.y, rayHit.point.z), Quaternion.Euler(new Vector3(0f, -90f, 0f)), 1f, EventType.MouseMove);

                        Handles.color = Color.green;

                        Handles.ConeHandleCap(0, new Vector3(rayHit.point.x, -1f + Origin.vector3Value.y, rayHit.point.z), Quaternion.Euler(new Vector3(-90f, 0f, 0f)), 1f, EventType.MouseMove);
                        Handles.ConeHandleCap(0, new Vector3(rayHit.point.x, fogOfWarManager.LevelHeight * PatchScale.floatValue + Origin.vector3Value.y + 1f, rayHit.point.z), Quaternion.Euler(new Vector3(90f, 0f, 0f)), 1f, EventType.MouseMove);
                        Handles.color = Color.white;
                    ...
                    if (NewCoordinate < FogOfWar.ResistanceMapData.Length && NewCoordinate >= 0)
                    {
                        ...
                        if (FogOfWar.ResistanceMapData[NewCoordinate].r == 0)
                        {
                            Handles.Label(rayHit.point + Camera.current.transform.rotation * new Vector3(0.025f, 0.078f, 0f) * CamDist, "Hardblock: no", EditorStyles.whiteMiniLabel);
                        }
                        else
                        ...
                        Handles.Label(rayHit.point + Camera.current.transform.rotation * new Vector3(0.025f, 0.061f, 0f) * CamDist, "Height: " + (FogOfWar.ResistanceMapData[NewCoordinate].g * 255).ToString(), EditorStyles.whiteMiniLabel);
    ...
    private void DrawPreviewFieldVertical(Vector3 _Position, int _PlaceMode)
    {
        ...
        for (float x = (InitialPoint.x - 4 * PatchScale.floatValue); x < (InitialPoint.x + 5 * PatchScale.floatValue); x += PatchScale.floatValue)
        {
            ...
            for (float y = (InitialPoint.y - 4 * PatchScale.floatValue); y < (InitialPoint.y + 5 * PatchScale.floatValue); y += PatchScale.floatValue)
            {
                ...
                Handles.RectangleHandleCap(0, new Vector3(x, y, _Position.z), Quaternion.identity, .49f * PatchScale.floatValue, EventType.MouseMove);
                ...
                if (coord >= FogOfWar.ResistanceMapData.Length || coord < 0)
                    return;

                if (_PlaceMode == 0)
                {
                    if (FogOfWar.ResistanceMapData[coord].r != 0f)
                    {
                        Handles.color = new Color(.953f, .659f, .157f, 1f);
                        Handles.RectangleHandleCap(0, new Vector3(x, y, _Position.z), Quaternion.identity, .38f * PatchScale.floatValue, EventType.Layout);
                        Handles.RectangleHandleCap(0, new Vector3(x, y, _Position.z), Quaternion.identity, .37f * PatchScale.floatValue, EventType.Layout);
                        Handles.RectangleHandleCap(0, new Vector3(x, y, _Position.z), Quaternion.identity, .36f * PatchScale.floatValue, EventType.Layout);

                        Handles.color = new Color(.5f, .2f, 0f, 1f);
                        Handles.RectangleHandleCap(0, new Vector3(x, y, _Position.z), Quaternion.identity, .35f * PatchScale.floatValue, EventType.Layout);
                        Handles.RectangleHandleCap(0, new Vector3(x, y, _Position.z), Quaternion.identity, .34f * PatchScale.floatValue, EventType.Layout);
                        Handles.RectangleHandleCap(0, new Vector3(x, y, _Position.z), Quaternion.identity, .33f * PatchScale.floatValue, EventType.Layout);
                    }
                }
                else if (_PlaceMode == 1)
                {
                    Handles.color = new Color(.953f, .659f, .157f, 1f);
                    float height = (fogOfWarManager.StartEndHeight.y - fogOfWarManager.StartEndHeight.x) * FogOfWar.ResistanceMapData[coord].g;
                ...
                {
                    if (FogOfWar.ResistanceMapData[coord].a != 1f)
                    {
                        Handles.color = new Color(.953f, .659f, .157f, 1f);
                        Handles.RectangleHandleCap(0, new Vector3(x, y, _Position.z), Quaternion.identity, .48f * PatchScale.floatValue, EventType.Layout);
                        Handles.RectangleHandleCap(0, new Vector3(x, y, _Position.z), Quaternion.identity, .47f * PatchScale.floatValue, EventType.Layout);
                        Handles.RectangleHandleCap(0, new Vector3(x, y, _Position.z), Quaternion.identity, .46f * PatchScale.floatValue, EventType.Layout);

                        Handles.color = new Color(.5f, 0f, 0f, 1f);
                        Handles.RectangleHandleCap(0, new Vector3(x, y, _Position.z), Quaternion.identity, .45f * PatchScale.floatValue, EventType.Layout);
                        Handles.RectangleHandleCap(0, new Vector3(x, y, _Position.z), Quaternion.identity, .44f * PatchScale.floatValue, EventType.Layout);
                        Handles.RectangleHandleCap(0, new Vector3(x, y, _Position.z), Quaternion.identity, .43f * PatchScale.floatValue, EventType.Layout);
                    }
                }
    ...
    private void SavePNGToDisc()
    {
        ...
        if (fogOfWarManager.ResitenceMapPath != null && fogOfWarManager.ResitenceMapPath != "")
        {
            byte[] pngData = fogOfWarManager.GetResistanceMap().EncodeToPNG();
            ...
            fogOfWarManager.SetResistanceMap(fogOfWarManager.ResitenceMapPath);
        }
    }
}
