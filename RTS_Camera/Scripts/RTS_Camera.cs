...
namespace RTS_Cam
{
    ...
    public class RTS_Camera : Photon.MonoBehaviour
    {
        ...
        public float mouseRotationSpeed = 0f;
        ...
        private void Update()
        {
            if (!useFixedUpdate)
                CameraUpdate();
            if (Input.GetKey(KeyCode.Space) && !CameraTempLock)
            {
                SetTarget(Player);
                CameraTempLock = true;
            ...
            if (Input.GetKeyDown(KeyCode.V) && !CameraLock)
            {
                SetTarget(Player);
                CameraLock = true;
            }
            else if (Input.GetKeyDown(KeyCode.V) && CameraLock)
            {
                ResetTarget();
                CameraLock = false;
                MinimapMark.transform.localPosition = Vector3.zero;
        ...
        private void FollowTarget()
        {
            ...
            MinimapMark.transform.localPosition = adjust;
        ...
        private void LimitPosition()
        {
            ...
            m_Transform.position = new Vector3(Mathf.Clamp(m_Transform.position.x, limitX + 5, limitX + 280),
                m_Transform.position.y,
                Mathf.Clamp(m_Transform.position.z, -limitY, limitY + 260));
        }
