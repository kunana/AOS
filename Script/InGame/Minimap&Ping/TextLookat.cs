...
public class TextLookat : MonoBehaviour
{
    ...
    private void FindSender(byte eventCode, object content, int senderId)
    {
        ...
        {   //핑 프리팹 이름, 월드 좌표, 샌더챔피언 이름, 샌더의 포지션
            ...
            if (datas.Length.Equals(4))
            {
                ...
                if (temp.Contains("Ashe") || temp.Contains("ashe"))
                {
                    temp = "JeAshe";
                }
                else if (temp.Contains("Mundo") || temp.Contains("mundo"))
                {
                    temp = "GeoMundo";
                }
                else if (temp.Contains("Alistar") || temp.Contains("alistar"))
                {
                    temp = "HaYeongsoo";
                }
                MyChampName = temp;
    ...
    private void Update()
    {
        ...
        if (!isLocal)
        {
            ...
            transform.GetChild(0).DOLookAt(SenderPlayerPos, 0.5f, AxisConstraint.Y); // Y축만 회전되게 고정
