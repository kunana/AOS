...
public class Shop : MonoBehaviour {
    ...
    public void MakingItemSearch(int itemID)
    {
        ...
        foreach (ShopItem.Item i in ShopItem.Instance.making_itemlist)
        {
            ...
            createdItem.GetComponent<Image>().sprite = Resources.Load<Sprite>("RPG icons/64X64/" + i.icon_name);
        }
    ...
    public void DescriptionUpdate(int itemID)
    {
        ...
        ShopItem.Item selectItem = ShopItem.Instance.itemlist[itemID];
        // 아이콘 이름 가격
        Sprite Icon_image = Resources.Load<Sprite>("RPG icons/64X64/" + selectItem.icon_name);
        ...
        if (selectItem.mana_regen != 0)
            stat_string += "기본 마나 재생 +" + selectItem.critical_percent.ToString() + "%\n";
